...
const Join = () => {
  ...
  const handleClickZipCode = () => {
    // 웹 브라우저에서 새창열기
    open({ onComplete: handleCompleteZip });
  };
  ...
  const handleCompleteZip = data => {
    ...
    // console.log("fullAddress : ", fullAddress); // 전체주소
  ...
  return (
    <div>
      <form onSubmit={handleSubmit(onSubmitJoin)}>
        ...
        <div>
          <label>주소검색</label>
          <button onClick={handleClickZipCode}>주소 검색</button>
        </div>
        <div>
          <label>상세 주소</label>
