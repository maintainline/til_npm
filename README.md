...
- full calendar 가 활용 많이 됨 (기능많음, 복잡함)
...
function Schedule() {
  ...
  // 1. 날짜를 US 방식으로 변경 (월화수~ 를 일월화수~ 로)
  const weekName = ["sun", "mon", "tue", "wed", "thu", "fri", "sat"];
  const formatShortWeekday = (locale, date) => {
    const idx = date.getDay();
    // Date 객체에서 getDay 는 날짜가 0:일, 1:월 2:화 ...
    // console.log(idx);
    return weekName[idx];
  };
  // jsx 자리
...
<Calendar
 ...
  tileClassName={tileClassNameShow} // show 추가
...
/>
```
