...
- 참조 https://iwoohaha.tistory.com/318My Project 41621
...
```txt
REACT_APP_GOOGLE_CLIENT_KEY=본인 클라이언트 id
REACT_APP_GOOGLE_SECRET_KEY=본인 키
```
...
export const getGoogleToken = async code => {
  ...
  const SECRET_KEY = import.meta.env.REACT_APP_GOOGLE_SECRET_KEY;
...
function AfterGoogle() {
  ...
  //구글의 사용자의 정보를 접근하는 토큰 즉, accessToken을 관리
  ...
  return (
    <div>
      <h1>구글 사용자 정보</h1>
      <p>인가 코드 : {authCode}</p>
      <p>액세스 토큰 : {accessToken ? "성공적으로 가져옴" : "없음"}</p>
      <div>
        {userInfo ? (
          <div>
            <p>아이디: {userInfo.id}</p>
