...
- html 이 입력이 되는 것은 `크로스 사이트 스크립트 공격` 가능성 있음.
- 줄여서 `XSS` 위험이 존재함.
- 위의 공격을 회피하기 위해서 추가 npm 설치가 필요함.(에디터 사용시 필수 설치)
...
const AddForm = () => {
  ...
  return (
    <div>
      ...
      <div>
        <p>입력중인 내용 : {data}</p>
        {/* 아래처럼 출력해 봅시다 : 리액트에서 위험한 내용이고 , html 형태이다.(설정함..?) */}
...
- 최종 실습은 Supabase 또는 Firebase, 백엔드 서버가 있어야함.
